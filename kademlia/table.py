...
class KTable(object):
    ...
    def append(self, node):
        ...
        try:
            ...
            if not bucket.inRange(self.nid):
                return

            self.splitBucket(index)
            self.append(node)

    def findCloseNodes(self, target, n=const.K):
        """
    ...
    def bucketIndex(self, target):
        ...
        """
        return bisect.bisect_left(self.buckets, intify(target))
...
class KBucket(object):
    ...
    def isFresh(self):
        """bucket是否新鲜"""
        return (time.time() - self.lastAccessed) > const.BUCKET_LIFETIME
